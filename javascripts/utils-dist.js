...
var removeUntranscribedPunctuationAndNormalize = function removeUntranscribedPunctuationAndNormalize(tibetan) {
  return tibetan.replace(/[༵\u0F04-\u0F0A\u0F0D-\u0F1F\u0F3A-\u0F3F\u0FBE-\uF269]/g, '').trim().replace(/ཿ/g, '་').replace(/་+/g, '་').replace(/་$/g, '').replace(/ༀ/g, 'ཨོཾ').replace(/[ྃྂ]/g, 'ཾ').replace(/(ཾ)([ཱཱཱེིོིྀུུ])/g, '$2$1') // Malformed: anusvara before vowel
