  ...
  return {
    ...
    find: function find(tibetan) {
      ...
      if (exception) {
        if (modifier) {
          if (modifier == 'ས') {
            if (exception.last() == 'a') exception = exception.slice(0, -1) + t('drengbu');else if (exception.last() == 'o') exception = exception.slice(0, -1) + t('ö');else if (exception.last() == 'u') exception = exception.slice(0, -1) + t('ü');else if (!['i', 'e', 'é', 'è'].includes(exception.last())) exception += modifier;
    ...
    tryException: function tryException(key) {
    ...
    removeDuplicateEndingLetters: function removeDuplicateEndingLetters(text) {
      return text.replace(/(.?)\1*$/, '$1');
    },
    transcribeTibetanParts: function transcribeTibetanParts(text) {
      ...
      if (nonTibetanPart) {
        ...
        if (rest) return result + this.transcribeTibetanParts(rest);else return result;
...
var normalizeExceptions = function normalizeExceptions(exceptions) {
  return _(exceptions).inject(function (hash, value, key) {
    if (key.trim().length) {
...
Exceptions.initializeFromStorage = function (callback) {
  ...
  Storage.get('general-exceptions', normalizeExceptions(defaultGeneralExceptions), function (value) {
...
Exceptions.initializeFromDefaults = function () {
  this.generalExceptions = normalizeExceptions(defaultGeneralExceptions);
};
...
Exceptions.updateGeneralExceptions = function (exceptions, callback) {
  ...
  Storage.set('general-exceptions', normalizedExceptions, function (value) {
